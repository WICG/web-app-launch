...
### Login Flows
Some authentication flows involve redirecting a user to an authentication provider, and redirecting back to the app once authenticated. If the navigation to the authentication provider is captured in a launch event and sent to an app window, it will break the authentication flow (redirecting back to the original site will happen in the authentication provider window.).

![Bad Login Flow](images/bad-login-flow.png)
> Hypothetical example where Discoverer authenticated with Spotify (which opened in an app window). Upon authentication, Discoverer is open in a custom tab inside Spotify.

There is a simple solution here: Launch events in authentication providers should not handle login links. This is workable because:
- Launch events are opt in (so nothing will break unless sites choose to add them)
- and providers have an incentive not to break login flows so third parties don't migrate to competitors.

As an added mitigation, in #10, a solution is discussed which would provide apps with a way of opting out of triggering launch events entirely, though it is out of scope for this explainer.
